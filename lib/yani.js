...
export function parseYummyTvHtml(html) {
  let doc = null;
  try {
  ...
  const pick = (selector, attribute = "textContent") => {
    const node = doc.querySelector(selector);
    if (!node) return "";
    return String(node.getAttribute(attribute) || node.textContent || "").trim();
  };
  const title = pick('[itemprop="name"]') || pick(".inner-page__title h1") || pick("h1") || pick("title");
  const altTitle = pick('[itemprop="alternativeHeadline"]') || pick(".inner-page__subtitle");
  const poster = doc.querySelector('link[itemprop="url contentUrl"]');
  const yearMatch = html.match(/<span itemprop="datePublished"[^>]*>[^<]*?(\d{4})/);
  return {
    title,
    altTitle,
    poster: poster ? new URL(poster.getAttribute("href"), "https://yummyanime.tv/").href : "",
    year: yearMatch ? Number(yearMatch[1]) : 0,
  };
}

export async function fetchYummyTvAnime(url, options = {}) {
  ...
  const parsed = parseYummyTvHtml(html) || { title: "", altTitle: "", poster: "", year: 0 };
...
export function groupVideos(videos) {
  ...
  for (const group of groups.values()) {
    ...
    group.episodes = group.kodik.length ? group.kodik : [];
