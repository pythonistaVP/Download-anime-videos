...
export async function fetchKodikLinks(iframeUrl, options = {}) {
  ...
  const html = await withRetry(
    async () => {
      const response = await fetchImpl(playerUrl, { headers: { Referer: referer }, signal });
  ...
  for (const [quality, list] of Object.entries(links)) {
    const first = Array.isArray(list) ? list.find((item) => item && item.src) : null;
    if (!first) continue;
    const decoded = decodeKodikLink(first.src);
    if (!decoded) continue;
    const numeric = Number(String(quality).replace(/[^\d]/g, ""));
    qualities[Number.isFinite(numeric) && numeric > 0 ? numeric : quality] = decoded;
  }
